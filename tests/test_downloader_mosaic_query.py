...
def _patch_relogin_and_sleep(monkeypatch, counter):
    monkeypatch.setattr(D, "_euclid_relogin",
                        lambda **k: counter.__setitem__("relogin",
                                                        counter["relogin"] + 1) or True)
    monkeypatch.setattr(D.time, "sleep", lambda *_a, **_k: None)


def test_query_returns_results(monkeypatch):
    c = {"relogin": 0}
    _patch_relogin_and_sleep(monkeypatch, c)
    monkeypatch.setattr(D.Euclid, "launch_job_async", lambda q: _FakeJob(["t1", "t2"]))
    res, err = D._query_mosaic_tiles("SELECT ...")
...
def test_none_then_success_retries_with_relogin(monkeypatch):
    ...
    res, err = D._query_mosaic_tiles("q", retries=2)
