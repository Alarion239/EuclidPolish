...
def captured_query(monkeypatch):
    ...
    def fake_async(query, *args, **kwargs):
        ...
        # exits cleanly.
        empty = Table(
            names=("right_ascension", "declination", "flux_vis_1fwhm_aper"),
            dtype=("f8", "f8", "f8"),
        )
...
def _flux_for_mag(mag: float) -> float:
    return 10 ** ((Config.DEFAULT_VIS_ZEROPOINT - mag) / 2.5)
...
def test_magnitude_min_emits_flux_upper_bound(tmp_path, captured_query):
    ...
    # Pull out the numeric upper bound following 'flux_vis_1fwhm_aper <'
    m = re.search(r"flux_vis_1fwhm_aper\s*<\s*([0-9.eE+\-]+)", q)
...
def test_magnitude_min_and_limit_together(tmp_path, captured_query):
    ...
    m_lo = re.search(r"flux_vis_1fwhm_aper\s*>\s*([0-9.eE+\-]+)", q)
    m_hi = re.search(r"flux_vis_1fwhm_aper\s*<\s*([0-9.eE+\-]+)", q)
    assert m_lo and m_hi
    # The "> 0" guard appears first in the WHERE; pick the one that matches
    # magnitude 21 (not 0).
    candidates = re.findall(r"flux_vis_1fwhm_aper\s*>\s*([0-9.eE+\-]+)", q)
...
def test_no_magnitude_min_omits_upper_bound(tmp_path, captured_query):
    ...
    q = captured_query["query"]
    # No '< <number>' clause for flux_vis_1fwhm_aper when magnitude_min unset.
    assert re.search(r"flux_vis_1fwhm_aper\s*<\s*[0-9]", q) is None, q
