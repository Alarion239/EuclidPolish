...
weight versus predicted VIS magnitude.  It keeps the raw TNG draw budget fixed
while moving probability out of the pre-render-faint tail and into the
23.5--25 magnitude range.  Outputs are diagnostics only: the Euclid side is MER
aperture photometry while the synthetic side is rendered truth flux, so a
common-detector validation is still required before enabling the curve in field
generation.
"""
...
def fit_log_weights(
    ...
    *,
    ...
    def residuals(parameters: np.ndarray) -> np.ndarray:
        ...
        # Match the requested 23.5--25 region most strongly, keep the bright
        # anchor fixed, and retain a smooth latent continuation beyond 25.
        importance = np.where(
            (centers >= 23.5) & (centers < 25.0),
            4.0,
            np.where(centers < 23.5, 2.0, 1.25),
        )
...
def _plot(
    ...
    ax.plot(
        ...
        linestyle=":", label="fit target",
    )
    ax.axvspan(23.5, 25.0, color="#008f5d", alpha=0.08)
    ax.set(
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    fit_region = (centers >= 23.5) & (centers < 25.0)
    # Never use the selected Euclid catalog to remove latent truth.  In the
    # requested fit region it is a lower bound on the real population.
    target_density[fit_region] = np.maximum(
        target_density[fit_region],
        euclid_density[fit_region],
    )
    ...
    payload: dict[str, Any] = {
        ...
        "selection_caveat": (
            "Synthetic values are rendered truth flux; Euclid values are MER "
            "detections with aperture photometry. Validate with common detector "
            "photometry before enabling in production generation."
        ),
        "inputs": {
