...
class FASRCPipelineStep(ABC):
    ...
    def build_sbatch_body(
        ...
        *,
        ...
        label: str,
        relative_log_dir: str = "logs/hst_pipeline",
    ) -> Dict[str, str]:
        """Render the full sbatch script, plus the relative log paths.

        Returns
        -------
        dict
            ``{"body": str, "script": rel, "out": rel, "err": rel, "name": str}``
            same shape as :func:`fasrc_jobs.build_sbatch_script` so the
            web submission helper can stay generic.
        """
        ts = time.strftime("%Y%m%d-%H%M%S")
        job_name   = f"hst-{self.step_id}-{ts}"
        ...
        body = textwrap.dedent(f"""\
            ...
            echo "============================================================"
            echo "HST pipeline step: {self.step_id} — {safe_label}"
            ...
            __HST_PIPELINE_T1__=$(date +%s)
            __HST_PIPELINE_RUNTIME__=$((__HST_PIPELINE_T1__ - __HST_PIPELINE_T0__))
            echo "============================================================"
            echo "Finished: $(date)"
            echo "RUNTIME_SECONDS=${{__HST_PIPELINE_RUNTIME__}}"
            echo "STEP_ID={self.step_id}"
            echo "============================================================"
        """)
        return {
            "body":   body,
