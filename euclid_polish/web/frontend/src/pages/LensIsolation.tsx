  ...
  return (
    <Page>
      <PageHead
        ...
        sub="Keep the deflector and lensed source. Remove every unrelated galaxy and star. Records and models stay in a sealed experiment namespace."
        right={<ConnectionBar />}
      />
        ...
        <CardBody>
          <div className="grid" style={{ gridTemplateColumns: "1fr auto 1fr", alignItems: "stretch", gap: "var(--s3)" }}>
            <div style={{ borderLeft: "4px solid var(--series-mean)", padding: "14px 16px", background: "var(--surface-2)" }}>
              ...
              <b>plain galaxies + complete lens + stars</b>
            </div>
      ...
      <div className="grid" style={{ gridTemplateColumns: "1fr", gap: "var(--s4)" }}>
        <Card>
          <CardHead title="1 · Generate paired layers" sub="50% complete lenses, 50% galaxy-rich negatives · CPU" right={<Badge>CPU</Badge>} />
          <CardBody>
            <div className="fasrc-step__res" style={{ marginBottom: "var(--s3)" }}>
              <NumberField label="train" value={ntrain} onChange={setNtrain} min={2} step={2} />
              <NumberField label="validate" value={nvalid} onChange={setNvalid} min={2} step={2} />
              <NumberField label="test" value={ntest} onChange={setNtest} min={2} step={2} />
        ...
        <Card>
          <CardHead title="2 · Fork and fine-tune" sub="One virgin experiment member per selected starless source · GPU" right={<Badge>GPU</Badge>} />
          <CardBody>
            <div className="fasrc-step__res" style={{ marginBottom: "var(--s3)" }}>
            ...
            {!sources.trim() && <p className="ui-field__hint">Enter at least one existing starless member before submitting.</p>}
            <StepById stepId="lens_isolation_train" extraParams={training} />
          </CardBody>
        </Card>

        <Card>
          <CardHead title="3 · Evaluate" sub="AUC, fixed-FPR recall, θE recall, flux retention, and hard-negative residuals" right={<Badge>GPU</Badge>} />
