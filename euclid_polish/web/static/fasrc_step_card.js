...
large cutout don't leak across train/validate."></label>`;
      ...
      case 'extract_euclid_psf':
        ...
        return `
          <label>Stars per PSF (N) <span class="muted">(average per cluster)</span>
            <input type="number" name="stars_per_psf" value="400" min="10" max="2000"
                   title="Target average stars per spatial cluster. K = round(n_good / N) clusters via K-Means++ on sky position, one ePSF per cluster. 3200 good stars at N=400 → ~8 lower-noise PSFs; elastic warps supply continuous diversity."></label>
          <label>Minimum stars per cluster
            <input type="number" name="min_stars_per_psf" value="200" min="1" max="2000"
  ...
  function _ensembleTrainFields() {
    ...
    return `
      ...
      <span data-mode-group="add fork continue">
        ...
        <label title="Examples per forward-modelled field. The eight random siblings are shuffled across optimizer batches.">
          Crops / field
          <input type="number" name="crops_per_field" value="8" min="1" max="64"></label>
        <label title="HR crop side; LR crops are half this size. 256 is the nearest scale-aligned half-side crop of a 510px field.">
          HR crop side
          <input type="number" name="hr_crop_size" value="256" min="2" max="508" step="2"></label>
        <label title="Four 256² crops contain about as many HR pixels as one 510² field while mixing parent exposures.">
          Batch size
  ...
  function _ensMemberRowHtml() {
    return `
      ...
      <label style="font-weight:normal;"
             title="Reconstruction loss on the asinh residuals. L1 is MAE, L2 is RMSE, L3 is the rooted cubic mean, and MSE is the unrooted mean squared error. (BerHu was tried and deprecated — no star resolution, worse PSNR/power-spectra, degenerate plateaus.)">
        loss
        <select data-f="loss">
          <option value="l1">L1 (median)</option>
          <option value="l2">L2 (RMSE)</option>
          <option value="l3">L3 (outlier-heavy)</option>
          <option value="mse">MSE (unrooted L2)</option>
        </select></label>
      <label style="font-weight:normal;"
