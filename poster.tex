...
\begin{columns}
  ...
  \begin{tikzpicture}[
    ...
    % Two lanes share a fixed horizontal offset so same-row panels are the
    % SAME size and line up. \lanesep = centre-to-centre of the left/right lane.
    \def\lanesep{0.42\linewidth}

    % ===== ROW 1: ingredients (left) + star-cluster map (right), equal size =====
    \node[panel] (ing)
        {\panelimg{0.31\linewidth}{\figdir/ingredients_grid.png}{ingredients grid}};
    \node[ptitle] at ([yshift=4mm]ing.north) {\large Ingredients};
    \node[panel] (cmap) at ([xshift=\lanesep]ing)
        {\panelimg{0.31\linewidth}{\figdir/cluster_map.png}{star-cluster map}};
    \node[ptitle] at ([yshift=4mm]cmap.north) {\large Star clusters};

    % ===== ROW 2: clean field (left) + per-band PSF grid (right), equal size =====
    \node[panel, anchor=north] (clean) at ([yshift=-26mm]ing.south)
        {\panelimg{0.36\linewidth}{\figdir/clean_field.png}{clean synthetic field}};
    ...
    \node[panel, anchor=north] (psf) at ([yshift=-26mm]cmap.south)
        {\panelimg{0.36\linewidth}{\figdir/psf_grid.png}{per-band PSFs}};

    % ===== ROW 3: mock-Euclid dirty grid (bigger, fed by clean + PSFs) =====
    \coordinate (mid) at ($(clean)!0.5!(psf)$);
    \node[panel, anchor=north] (dirty) at ([yshift=-32mm]clean.south-|mid)
        {\panelimg{0.46\linewidth}{\figdir/dirty_grid.png}{mock Euclid (4 bands)}};
    ...
    \node[panel, anchor=north] (sr) at ([yshift=-28mm]dirty.south-|mid)
        {\panelimg{0.42\linewidth}{\figdir/sr_field.png}{super-resolved}};
